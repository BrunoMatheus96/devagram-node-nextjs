...
const endpointSeguir = async (
    ...
    try {
        if (req.method === "PUT") {
            ...
            if (!usuarioLogado) {
                // Caso não retorne um usuário retorna um erro de usuário não encontrado
            ...
            //Busca se EU LOGADO sigo ou não esse usuario
            const jaESeguidor = await SeguidorModel.find({
                // Procura na DB de seguidor se já há uma correspondência de seguidor e seguido conforme os id's
                usuarioId: usuarioLogado._id,
                usuarioSeguidoId: usuarioASerSeguido._id,
            });
            if (jaESeguidor && jaESeguidor.length > 0) {
                // Caso já seja seguidor faremos o processo de unfollow
                jaESeguidor.forEach(
                    async (e: any) =>
                        await SeguidorModel.findByIdAndDelete({ _id: e._id }) // Para cada correspondencia que retornou procura ele no DB e deleta as correspondências uma a uma
                );
                ...
                await UsuarioModel.findByIdAndUpdate(
                    ...
                    usuarioLogado
                ); // Atualiza o usuário logado na DB
                await UsuarioModel.findByIdAndUpdate(
                    { _id: usuarioASerSeguido._id },
                    usuarioASerSeguido
                ); // Atualiza o usuário a ser desseguido na DB
                return res.status(200).json({ msg: "Usuário desseguido com sucesso" }); // Retorna uma mensagem de sucesso
            } else {
                // Caso não haja correspondência e o usuário ainda não segue o usuário a ser seguido faremos o processo de seguir
                const seguidor = {
                    // Cria um objeto seguidor conforme esperado no Model de seguidor
                    usuarioId: usuarioLogado._id,
                    usuarioSeguidoId: usuarioASerSeguido._id,
                };
                ...
                await UsuarioModel.findByIdAndUpdate(
                    { _id: usuarioASerSeguido._id },
                    usuarioASerSeguido
                ); // Atualiza o usuário a ser seguido na DB
        ...
        return res
            .status(500)
            .json({ erro: "Não foi possível seguir/desseguir o usuário informado" });
    }
};
